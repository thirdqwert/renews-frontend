...
const categoriesLinks = [
    ...
    { name: "Происшествия", slug: "proisshestviya", breakpoint: "hidden 4xl:block" },
...
export default function Header() {
    ...
    const [subcategoryIsOpenSidebar, setSubcategoryIsOpenSidebar] = useState<string | null>(categoryBy?.toString() || null)
            ...
            return (
                ...
                >
                    ...
                    <Swiper
                        ...
                        modules={[Navigation]}
                    >
                        {currentCateogry && currentCateogry.subcategories.map(subcategory => (
                            <SwiperSlide key={subcategory.id} >
                                <Link href={`/news/${currentCateogry.slug}/${subcategory.slug}`} className="block cursor-pointer bg-[#295480] hover:bg-[#f4f6f9] hover:text-[#295480] text-[#f4f6f9] text-[13px] md:text-[18px] font-medium py-[5px] px-[10px] rounded-[20px]">{subcategory.title}</Link>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                </div >
                ...
                <nav className="flex flex-row justify-between items-center">
                    <div className="flex flex-row items-center gap-[40px]">
                        ...
                        <ul className="hidden lg:flex flex-row gap-[30px]">
                            ...
                            <li onClick={() => setMoreCategoriesIsOpen(!moreCategoriesIsOpen)} onMouseEnter={() => setSubCategoryIsOpenHeader(null)} className="relative cursor-pointer flex flex-row gap-[5px]">
                                <span className="text-[13px] md:text-[18px] text-white font-medium">Ёще</span>
                                <Image src={arrowDown} alt="" />
                                {moreCategoriesIsOpen && <div
                                    className="fixed inset-0 z-40"
                                    onClick={() => setMoreCategoriesIsOpen(false)}
                                />}
                                {moreCategoriesIsOpen && <div className="absolute z-50 lg:top-[34px] xl:top-[40px] bg-[#343a40] w-[200px] flex flex-col pb-[10px] rounded-b-[10px]">
                                    <Link href="/news/nauka" className="text-[13px] md:text-[18px] text-white font-medium py-[5px] px-[10px] hover:bg-[#295480] hover:text-white">
                                        Наука
                                    </Link>
                                    <Link href="/news/kultura" className="block 7xl:hidden text-[13px] md:text-[18px] text-white font-medium py-[5px] px-[10px] hover:bg-[#295480] hover:text-white">
                                        Культура
                                    </Link>
                                    <Link href="/news/ekonomika" className="block 6xl:hidden text-[13px] md:text-[18px] text-white font-medium py-[5px] px-[10px] hover:bg-[#295480] hover:text-white">
                                        Экономика
                                    </Link>
                                    <Link href="/news/sport" className="block 5xl:hidden text-[13px] md:text-[18px] text-white font-medium py-[5px] px-[10px] hover:bg-[#295480] hover:text-white">
                                        Спорт
                                    </Link>
                                    <Link href="/news/proisshestviya" className="block 4xl:hidden text-[13px] md:text-[18px] text-white font-medium py-[5px] px-[10px] hover:bg-[#295480] hover:text-white">
                                        Происшествия
                                    </Link>
                                </div>
                                }
                    ...
                    <div className="flex flex-row items-center gap-[10px]">
                        <Link href={"/news/search/"} className="pr-[0px] md:pr-[10px] md:w-[30px] h-[20px] order-1"><Image width={20} height={20} src={SearchIcon} alt="Поиск" /></Link>
                        <div className="hidden ssm:block">
                            <ExchangeRate />
                        </div>
                        <Image src={BurgerIcon} alt="" className="cursor-pointer md:block lg:hidden order-2" onClick={() => setSidebarIsOpen(true)} />
                    </div>
                </nav>
            </header>
            {/* Sidebar */}
            {sidebarIsOpen && <div onClick={() => setSidebarIsOpen(false)} className="fixed top-0 bottom-0 right-0 left-0 bg-[rgba(29,29,29,0.4)] z-[998]" />}
            <div
            ...
            >
                <div className="flex flex-col gap-[15px] bg-[#343a40] px-[15px] py-[17px] md:py-[25px]">
                    <div className="flex flex-row justify-between items-center">
                        ...
                        <Image onClick={() => setSidebarIsOpen(false)} src={CloseIcon} alt="" className="w-[30px] h-[25px] object-scale-down bg-[#5b5757] rounded-[5px]" />
                    </div>
                ...
                <div className="">
                    ...
                    <div className="pl-[30px]">
                        <Link href={"/"} className="flex flex-row gap-[17px] items-center py-[15px]" onClick={() => setSidebarIsOpen(false)}>
                            <Image src={homeIcon} alt="" className="w-[22px] object-scale-down" />
                            <span className="text-[16px] font-medium text-[#212529]">Главная</span>
                        </Link>
                        <Link href={"/news/"} className="flex flex-row gap-[17px] items-center py-[15px]" onClick={() => setSidebarIsOpen(false)}>
                            <Image src={newsIcon} alt="" className="w-[22px] object-scale-down" />
                            <span className="text-[16px] font-medium text-[#212529]">Новости</span>
                        </Link>
                        <Link href={"/contacts/"} className="flex flex-row gap-[17px] items-center py-[15px]" onClick={() => setSidebarIsOpen(false)}>
                    ...
                    <ul className="pt-[27px]">
                        {categories && categories.map(category => (
                            <div key={category.id} className="py-[10px] pl-[30px] cursor-pointer pr-[14px]">
                                <li className="flex flex-row justify-between items-center" onClick={() => setSubcategoryIsOpenSidebar(category.slug)}>
                                    <Link className="text-[18px] text-[#212529] font-medium" href={`/news/${category.slug}/`} >{category.title}</Link>
                                    <Image src={subcategoryIsOpenSidebar == category.slug ? blueArrowDown : arrowRight} alt="" />
                                </li>
                                {subcategoryIsOpenSidebar == category.slug && <ul className="vertical_line_blue flex !flex-col mt-[15px]">
                                    {category.subcategories.map(subcategory => {
                                        if (category.title != "Реклама") return (
                                            <Link key={subcategory.id} href={`/news/${category.slug}/${subcategory.slug}/`} className="flex flex-row items-center gap-[10px] px-[10px] py-[10px]">
                                                <span className="h-[10px] min-w-[10px] rounded-full" style={{ background: subcategory.slug == subcategoryBy ? "#29547f" : "#a8a0a0" }} />
                                                <span className="text-[18px] text-[#212529] font-medium" >{subcategory.title}</span>
                                            </Link>
                                        )
                                    })}
                                </ul>}
