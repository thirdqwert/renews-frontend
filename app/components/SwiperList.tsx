                    ...
                    {list.map(item => (
                        <SwiperSlide key={item.id} className="rounded-[20px] overflow-hidden relative">
                            <Link href={`/news/detail/${item.id}`}>
                                <Image unoptimized width={0} height={0} className="block h-full w-full align-middle object-cover" src={item.preview} alt={item.short_title} />
                                <div
                                ...
                                <article className="absolute bottom-[35px] px-[5px] md:px-[32px] lg:px-[48px] xl:px-[80px]">
                                    <h3 className="text-[11px] md:text-[19px] lg:text-[25px] xl:text-[35px] text-white font-bold short_title">{item.title}</h3>
                                    <div className="flex flex-row gap-[30px] items-end">
                                        <span className="text-[9px] md:text-[16px] lg:text-[20px] text-white font-medium">{item.category}</span>
                                        <time className="text-[9px] md:text-[16px] lg:text-[20px] text-white font-medium" dateTime={item.created_at.split('T')[0]}>{getDateString(item.created_at)}</time>
                                    </div>
                                </article>
                            </Link>
                        </SwiperSlide>
